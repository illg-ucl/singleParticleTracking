function spot = findSpotCentre1frame(image_data,x_estimate,y_estimate,subarray_halfsize,inner_radius,sigma_gauss,guess_sigma_fit)
% 
...
if noConvergenceFlag == 1 % if method did not converge:
    ...
    spot.SigmaFit = 0; 
    spot.I0Fit = 0; 
    spot.bg_noise_offset_afterBGsubtract = 0;
    spot.BgNoiseStd = 0; 
    spot.IbgAvg = 0;
    spot.IbgTot = 0; 
    spot.SNR = 0;
...
% if spot.ClipFlag == 0
%     %     && spot.SigmaFit<4 && spot.SigmaFit>2 && spot.SNR>=2.7
%     figure;
%     plot(r_matrix,data_to_fit,'*');
%     hold on;
%     plot(fit_result);
%     hold off; % check fit result graphically.
% end

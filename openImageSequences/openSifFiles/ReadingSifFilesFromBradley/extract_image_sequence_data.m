...
%
%
% Read image sequence (.sif or .dv data) and return image data in a useful
% form.
%
% Inputs: 
...
% Error control if neither a .sif, .tif or .dv file can be found with that label:
if isempty(dvImagePath0) && isempty(sifImagePath0) && isempty(tifImagePath0) && isempty(matImagePath0) % If there is no .sif, .tif or .dv image sequence file for such image_label, show error and exit function:
    error('Check you are in the correct directory and run again. No .sif, .tif or .dv file found for that image_label.');
end

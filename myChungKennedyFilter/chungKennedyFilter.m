...
% publications.
%
% Calculate a moving average with two windows of width W (W is the no. of data points in the window), one to the left 
...
%
% Works well with W = 5-15 and R = 50 for Oxphos data (Isabel).
%
% Uniform sampling of vector X is assumed (i.e., no jumps in the x axis).
%
% Outputs:
...
% the window size plus one:
if N < (W+1)    
    disp(['The number of intensity data points is: ',num2str(length(X))])
    disp(['The filter window size (data points) is: ',num2str(W)])
    XX = []; TX = []; DX = []; SD = []; DSD = []; XPRE = [];
    warning('Window size for Chung-Kennedy filter cannot be larger than number of data points minus one');
    % return    
    disp(' ');
    disp('Warning: filter window size has been set to the number of data points minus one.')
    W = N-1;
end

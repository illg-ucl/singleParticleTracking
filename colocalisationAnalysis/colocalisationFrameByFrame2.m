function result_struct = colocalisationFrameByFrame2(image_label,bf_image_label,start_frame,end_frame,start_frame_forFrameAvg,end_frame_forFrameAvg,roi,output_label,colocalis_threshold,factor_forThresholding,singleFrame_topThreshold,singleFrame_bottomThreshold)
%
% Created by Isabel Llorente-Garcia, September 2012.
...
% fluorescence images.
% Similar to colocalisationFrameByFrame.m but applying a threshold (coming
% from background autofluorescent levels in parental strains) to get signal
% masks in each channel. Also the mask is calculated for each frame as
% opposed to for the frame average as in colocalisationFrameByFrame.m. So
% it is possible that the mask varies slightly from frame to frame in this
% function.
%
% See
...
% - singleFrame_topThreshold and singleFrame_bottomThreshold: default
% values are 300 and 500 respectively for OXPHOS Isabel data.
% % Absolute values for thresholding top and bottom channels (single frames)
% before re-scaling them. These come from average values of
% autofluorescence in the cell in each channel, at the beginning of the
% image sequence, on the parental (no flu labels) strains.
%
%
% Example of how to call this function:
% rs498 = colocalisationFrameByFrame2('498','497',5,30,5,500,'full_image',' ') for fluorescence image sequence 498, finding the displacement between top and bottom channels from bright field image '497'. .
...
% rs498 = colocalisationFrameByFrame2('498','497',7,7+25,7,'end',[307 384 37 129],'')
%
% Output: 
...
% Region of interest as given by the input "roi":
xleft = roi(1);
xright = roi(2);
ytop = roi(3);
ybottom = roi(4);
...
% full_signalMask_0 = (SignalMaskTop2 | SignalMaskBot); % union of signal masks of top and bottom channels.
% full_bgndMask_0 = (BgndMaskTop2 & BgndMaskBot); % intersection of background masks of top and bottom channels.
...
full_signalMask_0 = SignalMaskTop2;
full_bgndMask_0 = BgndMaskTop2;
...
for n = start_frame:end_frame % loop through chosen frames
    ...
    % and convert to grayscale (0 to 1) images (so final result is only
    % qualitative):
    Btop00_singleFrame = frame(1:half,:); % top channel.
    Bbot00_singleFrame = frame((half+1):whole,:); % bottom channel.
    % "00" at end means full image before taking the roi.
    
    % 3 -  Shift the top image:
    % Shift the top channel to counteract slight displacement between channels:
    % eg. [5,-4]: shift down 5 pixels and left 4 pixels.
    % Shifted top image:
    Btop002_singleFrame = circshift(Btop00_singleFrame,translateBy); % shift top image by vector translateBy.
    % "0" at end means full image before taking the roi.
    result1_singleFrame = Btop002_singleFrame-Bbot00_singleFrame; % shifted top image minus bottom one.
    
    % 4 - Use a region of interest on the frame average:
    % The region of interest is given by the input "roi".
    % Use previous values of xleft,xright,ytop,ybottom.
    if strcmp(roi,'full_image') % if input "roi" is 'full_image'
        Btop02_singleFrame = Btop002_singleFrame;
        Bbot0_singleFrame = Bbot00_singleFrame;
    else % if input "roi" is 'full_image' a row vector [xleft xright ytop ybottom]
        Btop02_singleFrame = Btop002_singleFrame(ytop:ybottom,xleft:xright);
        Bbot0_singleFrame = Bbot00_singleFrame(ytop:ybottom,xleft:xright);
    end
    % I keep the "2" at the end of Btop to indicate that it is the shifted top channel.
    
    % Do thresholding to re-scale images. Use levels of bgnd
    % autofluorescence of cells in each channel (obtained from parental
    % strains) to do this thresholding. Build a mask for each channel.
    % Note that these extra masks might change for each frame...
    thresh_mask_top = (Btop02_singleFrame>singleFrame_topThreshold);
    thresh_mask_bot = (Bbot0_singleFrame>singleFrame_bottomThreshold);
    
    full_signalMask = full_signalMask_0.*(thresh_mask_top | thresh_mask_bot); % union of signal masks of top and bottom channels, and only select those values within the mask coming from the frame average (_0).
    full_bgndMask = ~full_signalMask;
  
    
    % 5 - Re-scale images to make them between 0 and 1 so that the threshold is 0 and the maximum is 1:
    % Note that the re-scaling is done after shifting and taking the roi.
    Btop2_singleFrame = thresh_mask_top.*(Btop02_singleFrame-singleFrame_topThreshold)/(max(max(Btop02_singleFrame))-singleFrame_topThreshold); 
    Bbot_singleFrame = thresh_mask_bot.*(Bbot0_singleFrame-singleFrame_bottomThreshold)/(max(max(Bbot0_singleFrame))-singleFrame_bottomThreshold); 
...
%     subplot(3,2,5); imshow(result2,[-max(abs(min1),max1),max(abs(min1),max1)]);
    subplot(3,2,5); imshow(result2,[-1,1]);
...
subplot(1,2,1)
bar(binCentres,freqCounts,'cyan')
...
plot(vector_x,vector_y_left,'r','LineWidth',1.5) % red left
plot(vector_x,vector_y_centre,'g','LineWidth',1.5) % green central
plot(vector_x,vector_y_right,'b','LineWidth',1.5) % right blue
legend('','sum3Gaussians','left','central','right','Location','Best')

function plotTrajOnSequence(data_set_label,image_label,traj_numbers,pause_seconds)
...
% and shows the image frames together with the trajectory results overlaid.
...
% plotTrajOnSequence('GFP-nuoF & mCherry-sdhC','470',[1:48],0.5) plots
...
% Another example: plotTrajOnSequence('cybD-mCherry &ATPase-GFp','490',[16 17 18],0.5)
...
imshow(imrotate(framebis,90),[],'Border','tight','InitialMagnification',150); % rotate image by 90 degrees and show it scaled between its min and max values ([]).
hold;
disp(['frame number: ',num2str(k)]) % print frame number to Command Window.
    ...
    select_traj_row = find(all_traj_data{m}(:,1)==k); % find row of trajectory data values that corresponds to frame k.
        if length(select_traj_row)==1
             ...
             plot(top_x_data,top_y_data,'o','Color',color_list(m,:),'MarkerSize',10);
             plot(bottom_x_data,bottom_y_data,'o','Color',color_list(m,:),'MarkerSize',10);
        end     
    end
    hold off;

function linkTrajSegments(image_label,start_frame,end_frame,spot_results,data_set_label)
% 
% Created by Isabel Llorente-Garcia, Sept. 2011.
% If you use this code please acknowledge Isabel Llorente-Garcia in your
% publications.
%
% This function links and groups all found trajectory segments (of bright fluorescent spots) 
...
% They should be the same as previously used to produce spot_results using FindTrajects.m.
% spot_results: parameters (spot_results{1}) and calculated trajectory segments (spot_results{2}). It is the output of function FindTrajects.m 
...
% linkTrajSegments('490',100,110,s1,'ATPase-GFP'),
% where s1 =FindTrajects('490',100,110) has been previously calculated;
...
% I have saved all parameter values in the file "paramsForLinkTrajSegments.m"
...
%% Extract segment (small trajectory) results from "spot_results" input:

params_for_FindTrajects = spot_results{1}; % parameters used in function FindTrajects.m to obtain segments.
traj_results = spot_results{2}; % Results of segments, obtained by function FindTrajects.m.
...
% data arranged in a large matrix (spot after spot).

max_no_spots = size(traj_results,2); % max. number of found spot centres in each frame.

data_to_export_labels = fieldnames(traj_results)'; % Row vector with the fieldnames of all found spots.
% ('CentreX','CentreY','IspTot','rsqFit','SigmaFit','I0Fit','BgNoiseStd','IbgAvg','IbgTot','SNR','IinnerTot','ClipFlag','TrajNumber','FrameNumber','SpotNumber').

% Make of class single to save memory space:
all_data_to_export = single([]); % empty, to be filled in (original data, all spots).
data_to_export = single([]); % empty, to be filled in (only spots that have been linked into trajectories will be saved to this variable).
...
IspTot_column = strmatch('IspTot',data_to_export_labels); % number of column containing IspTot (used later).
Sigma_column = strmatch('SigmaFit',data_to_export_labels); % number of column containing SigmaFit (used later).
...
%     % Plot result trajectories:
    for q = 1:max_no_spots % loop through found spots on each frame:

        % traj_results(k,q) is a structure containing the found spot
        % characteristics, including fields 'CentreX', 'CentreY' and
        % 'TrajNumber'.
        tr = traj_results(k,q).TrajNumber; % trajectory number corresponding to found spot q on frame k.
        
        if ~isempty(traj_results(k,q).FrameNumber) % if the spot is not empty, save it to all_data_to_export.
            ...
            all_data_to_export = [all_data_to_export; single(cell2mat(struct2cell(traj_results(k,q)))')]; % append row with the spot data.
        end
        
        if  tr > 0  % only plot trajectory numbers > 0 (TrajNumber=0 is for loose spots):
...
%             plot(traj_results(k,q).CentreX,traj_results(k,q).CentreY,'o','Color',color_list(tr,:),'MarkerSize',10)
            ...
            data_to_export = [data_to_export; single(cell2mat(struct2cell(traj_results(k,q)))')]; % append row with the spot data.
        end
    end
%     pause(0.2);
%     hold off;
...
% Sort spot data by trajectory number and separate trajectories:
...
% Sort spots data by trajectory number:
if ~isempty(data_to_export)
    ...
    disp('!!')
    disp('WARNING: spots found were not linked into trajectories. No trajectories of more than one point were found.')
    disp('No excel file of trajectory results was saved.')
    disp('!!')
    return
end
...
% Error control: if there is only one trajectory number (traj number 1 or 0 for loose spots):
if unique([traj_results.TrajNumber])<=1
    ...
    % the previous data contains only spots which have been linked into a
    % trajectory.
    
    % Export also all (non-empty) spots in the input (traj_results) structure, even if they have not been linked into trajectories, i.e.,
    ...
    output_filename = strcat(data_set_label,'_',image_label,'allspots.xls'); % output .xls filename (before sorting data by trajectory number)
...
SigmaRatio01 = num2cell(A); % Note: SigmaRatio01 is a cell array (matrix) but SigmaRatio_01 below is a scalar.
...
% Iratio01 = cell(trajs_max,trajs_max); % Note: Iratio01 is a cell array (matrix) but Iratio_01 below is a scalar.
% SigmaRatio01 = cell(trajs_max,trajs_max); % Note: SigmaRatio01 is a cell array (matrix) but SigmaRatio_01 below is a scalar.
...
for qend = 1:trajs_max % loop though end points of all trajectories.
    ...
    for qstart = 1:trajs_max % loop though start points of all trajectories.
        ...
        x_end = separated_trajs{qend}(end,x_column); % CentreX position for last spot in trajectory qend.
        y_end = separated_trajs{qend}(end,y_column); % CentreY position for last spot in trajectory qend.
        IspTot_end = separated_trajs{qend}(end,IspTot_column); % IspTot for last spot in trajectory qend.
        Sigma_end = separated_trajs{qend}(end,Sigma_column); % SigmaFit for last spot in trajectory qend.
        Frame_end = separated_trajs{qend}(end,Frame_column); % frame number for last spot in trajectory qend.
        
        x_start = separated_trajs{qstart}(1,x_column);   % CentreX position for first spot in trajectory qstart.
        y_start = separated_trajs{qstart}(1,y_column);   % CentreY position for first spot in trajectory qstart.
        IspTot_start = separated_trajs{qstart}(1,IspTot_column); % IspTot for first spot in trajectory qstart.
        Sigma_start = separated_trajs{qstart}(1,Sigma_column); % SigmaFit for first spot in trajectory qstart.
        Frame_start = separated_trajs{qstart}(1,Frame_column); % frame number for first spot in trajectory qstart.
        
        % d_01: distance between end spot in trajectory qend and start point in trajectory qstart:
        d_01 = sqrt((x_end-x_start)^2+(y_end-y_start)^2);
        % Iratio_01: ratio of intensities of spot centre in previous and current frames:
        Iratio_01 = IspTot_end/IspTot_start;
        % SigmaRatio_01: ratio of widths of spots (Gaussian fits) in previous and current frames:
        SigmaRatio_01 = Sigma_end/Sigma_start;
        % Distance in frames (proportional to time) between end spot in trajectory qend and start point in trajectory qstart:
        ...
        % Accept asignment (link trajectory segments) and save values only if start and end spots in
        ...
        if d_01 < d_01_max && ...  % see PARAMETERS at start of this function.
            ...
            Iratio01{qend,qstart} = Iratio_01;
            SigmaRatio01{qend,qstart} = SigmaRatio_01;
            ...
            Iratio01{qend,qstart} = single(rej); % Use rej for asignments not accepted.
            SigmaRatio01{qend,qstart} = single(rej); % Use rej for rejected asignments.
...
% --------------------------------------------------------
% GROUP BEST ASSIGNMENTS WHICH CORRESPOND TO THE SAME SPOT:
...
%     k
    ...
    for q = 1:max_no_spots % loop through found spots on each frame:
        ...
        % full_traj_results(k,q) is a structure containing the found spot
        % characteristics, including fields 'CentreX', 'CentreY' and
        % 'TrajNumber'.
        tr = traj_results(k,q).TrajNumber; % trajectory number corresponding to found spot q on frame k (in old traj results).
        
        %         tr
        
        if isempty(old_new_traj_numbers)==0 % if there are traj numbers to be changed:
            if isempty(tr) == 0 % if that spot is not empty (due to padding of structure array):
                pos_to_change = find(old_new_traj_numbers(:,1)==tr); % check if traj number tr for this spot is contained in the list of traj numbers that need to be changed (old_new_traj_numbers).
                
                %             pos_to_change
                
                if isempty(pos_to_change) == 0 % if the traj number tr for this spot is contained in the list of traj numbers that need to be changed:
        ...
        if  tr > 0  % only plot trajectory numbers > 0 (TrajNumber = 0 is for loose spots):
            ...
            %             % Construct trajectory data to save (convert structure to matrix):
            %             data_to_export = [data_to_export; cell2mat(struct2cell(traj_results(k,q)))']; % append row with the spot data.
        end
    end
    pause(0.2);
    hold off;
...
% % Export also all (non-empty) spots in the input (traj_results) structure, 

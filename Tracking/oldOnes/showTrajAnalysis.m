function processedTraj = showTrajAnalysis(trajXlsPath,image_data,analysedAllTraj,n_traj,start_frame,tsamp,pixelsize_nm) 
%
% Isabel, Oct 2011.
...
r = numFrames-start_frame; % number of frames to average over, starting from start_frame. (20)

% PARAMETERs for default cell region around trajectory (for calculation of local cell coordinates):
xsize_default_cell_region = 120; % horizontal size of region around cell. 
ysize_default_cell_region = 120; % horizontal size of region around cell. 
...
% Plot default cell region corners and centre (for calculation of local cell coordinates):
...
if use_default_cell_region == 0
    ...
    if mean(x_values) < 268 % cell on the left
        local_cell_region = [216 268 268 324]; % values entered by hand for a given image sequence.
    else % cell on the right
        local_cell_region = [266 323 313 357]; 
    end
...
def_cell_region_x = [xleft,xleft,xright,xright]; % centre and corners of default cell region, x values.
def_cell_region_y = [ytop,ybottom,ytop,ybottom]; % centre and corners of default cell region, y values.
...
subplot(3,3,2);plot(tforFit,IforFit,'gX'); 
hold on; 
...
saturating_function = fittype('A*(1-exp(-t/B))','independent','t'); % define linear funtion to fit to, with 'x' as independent variable;
options = fitoptions('Method','NonlinearLeastSquares');  % Creates a structure of fit options with fields StartPoint, Lower, Upper, etc.
% CHECK: values of guesses:
...
try
    ...
    try
        ...
        options.StartPoint = [max(ydata_msd) guess_B]; % use a different guess for the saturating limit.
        [fit_msd_conf gof] = fit(xdata_msd,ydata_msd,saturating_function,options);
        fit_param_values = coeffvalues(fit_msd_conf); % parameter values resulting from fit. First one is 'A', second one is 'B'.
        fit_msd_conf_limit = fit_param_values(1); % offset from fit. This is the initial intensity, I at t_origin or tstart.
        fit_msd_conf_timeconst = fit_param_values(2); % slope from fit.
...
disp([' fit_msd_conf_limit = ',num2str(fit_msd_conf_limit),' +- ',num2str(fit_msd_conf_limit_stDev),';   fit_msd_conf_timeconst = ',num2str(fit_msd_conf_timeconst),' +- ',num2str(fit_msd_conf_timeconst_stDev)]) 
...
processedTraj{n_trackData}.intensity = analysedAllTraj(n_traj).intensity;
processedTraj{n_trackData}.filtered_I = filtered_I;

function processedTraj = showTrajAnalysisExtra(trajXlsPath,image_data,analysedAllTraj,n_traj,start_frame,tsamp,pixelsize_nm,n_ExtraFrames) 
%
% Created by Isabel Llorente-Garcia, Oct 2011.
% If you use this code please acknowledge Isabel Llorente-Garcia in your
% publications.
%
% This function is similar to showTrajAnalysis2.m, but the "Extra"
...
%
% Further analyse and process trajectory data (only traj number n_traj) for a given image sequence:
...
%  
% NOTE: this function works for short tracks too (as well as showTrajAnalysis2.m). (showTrajAnalysis.m does not!)
...
% - 'image_data': image sequence data, structure array with each element being a 2D matrix with 
...
%     'traj_in_pole'
%
% Note: function "showManyTrajAnalysisExtra.m" calls this function "showTrajAnalysisExtra.m".
%
...
% and showTrajAnalysisExtra(trajXlsPath,image_data,analysedAllTraj,n_traj,start_frame,tsamp(1),pixelsize_nm(1),n_ExtraFrames) for quick results (time in frames, size in pixels, uncalibrated).
%
...
% sampling_time = 0.04; minPointsTraj = 3; pixelToNM = 35.55;
% t1 = analyseTraj(pathTraj,sampling_time,minPointsTraj);
% procTraj = showTrajAnalysisExtra(pathTraj,'89',t1,1,4,sampling_time,pixelToNM,5);
% But usually it is best to call it from showManyTrajAnalysisExtra.m
...
% Note: the image sequence data is read within showManyTrajAnalysisExtra.m.


%% Read in the excel file with all tracks (filename ending in "...fullTrajs.xls") to
%% extract the parameters for finding spots and the size of regions used
%% for calculating spot intensity:

% This is used later to extend the trajectory after the tracking has
% finished, to try and capture the background level for a number of extra
% frames equal to n_ExtraFrames.

% Read in data in Sheet "params FindTrajects" from excel file at path "trajXlsPath" (input):
% Import the data in the sheet named 'params FindTrajects':
[numeric,txt,raw] = xlsread(trajXlsPath,'params FindTrajects');
...
% Extract parameters which give size of regions used to calculate spot intensity:
subarray_halfwidth = str_TrackInfo.subarray_halfwidth;
inner_circle_radius = str_TrackInfo.inner_circle_radius;
gauss_mask_sigma = str_TrackInfo.gauss_mask_sigma;
...
% Fit Y coordinates versus X coordinates (of previous cell mask region) to a line to find angle for coordinate transformation:
...
coord_transform_angle = -atan(cell_region_slope); % the angle is the arctan of the slope, in radians here.
...
%% GENERATE EXTENDED TRACK: 

% Extend trajectory after the tracking has finished, to capture the
% background level at the end of the track for a number of extra frames
% equal to n_ExtraFrames.

% Original track data:
I_originalTrack = analysedAllTraj(n_traj).intensity; % Column vector, Intensity is background corrected (see IspTot in 'findSpotCentre1frame.m').
t_originalTrack = analysedAllTraj(n_traj).timeabs; % Column vector, absolute time (comes from frame number and time between frames).
frameNum_originalTrack = analysedAllTraj(n_traj).frame; % Column vector, original track frame number.

% Reminder: original trajectory position data is:
% x_values = analysedAllTraj(n_traj).xvalues0; % list of original x centres of spots in trajectory n_traj.
% y_values = analysedAllTraj(n_traj).yvalues0; % list of original y centres of spots in trajectory n_traj.

% Stay at last tracked position and get intensity there to extend track:
x0 = x_values(end);
y0 = y_values(end);

% Parameters which give sizes of regions used to calculate spot intensity
% in function FindTrajects.m are:
% subarray_halfwidth
% inner_circle_radius

% Extend track, extra vectors:
frameNum_Extra = []; % initialise empty vector.
for kk = 1:n_ExtraFrames
    extra_frame_number = frameNum_originalTrack(end) + kk;
    if extra_frame_number <= numFrames % Error control, make sure we don't try to extend the track past the last frame.
    frameNum_Extra = [frameNum_Extra; extra_frame_number]; % column vector, extra frame numbers.
    end
end
t_Extra = tsamp*frameNum_Extra; % column vector, extra absolute time.
I_Extra = zeros(length(frameNum_Extra),1); % column vector, initialise empty row vector extra intensity values.

for n = frameNum_Extra(1):frameNum_Extra(end)  % loop through extra frames at end of track.
    m = n-frameNum_Extra(1)+1; % index which indicates position in vector of Extras (1:n_ExtraFrames).
    % Get frame data: 
    frame = image_data(n).frame_data; % extract frame data, stored in the field 'frame_data'.
    frame_data = double(frame);
    spot_Extra = calculateSpotIntensity(frame_data,x0,y0,subarray_halfwidth,inner_circle_radius);
    I_Extra(m,1) = spot_Extra.IspTot;
end

% Put together original track and extra bits into a extended track:
I_extendedTrack = [I_originalTrack; I_Extra];
t_extendedTrack = [t_originalTrack; t_Extra];
frameNum_extendedTrack = [frameNum_originalTrack; frameNum_Extra];
x_values_extendedTrack = [x_values; repmat(x0,[length(frameNum_Extra) 1])];
y_values_extendedTrack = [y_values; repmat(y0,[length(frameNum_Extra) 1])];

% Save results for extended track to output structure, all column vectors:
results_extendedTrack.I_extendedTrack = I_extendedTrack;
results_extendedTrack.t_extendedTrack = t_extendedTrack;
results_extendedTrack.frameNum_extendedTrack = frameNum_extendedTrack;
results_extendedTrack.x_values_extendedTrack = x_values_extendedTrack;
results_extendedTrack.y_values_extendedTrack = y_values_extendedTrack;

% Plot extended track and save as an additional png:
figure
% Plot xvalues and yvalues (origin is first point in trajectory) versus time:
subplot(2,2,1);
plot(t_extendedTrack,pixelsize_nm*(x_values_extendedTrack-x_values_extendedTrack(1)),'.-b'); 
hold on;
plot(t_extendedTrack,pixelsize_nm*(y_values_extendedTrack-y_values_extendedTrack(1)),'.-g');
xlabel('t_{abs} (s)'); 
ylabel('x-x0 (nm)(b),  y-y0 (nm)(g)');
% xlim([0 max(t_extendedTrack)]);
% legend('x-x0 (pix)','y-y0 (pix)');
legend('hide');
% display image file path:
axes_limits = axis; % get axes limits: axes_limits = [xmin xmax ymin ymax].
...
% Plot trajectory in x-y plane (origin is first point in trajectory), axes in nm now:
subplot(2,2,2);
plot(pixelsize_nm*(x_values_extendedTrack-x_values_extendedTrack(1)),pixelsize_nm*(y_values_extendedTrack-y_values_extendedTrack(1)),'.-k');
xlabel('x-x0 (nm)'); 
ylabel('y-y0 (nm)'); 
...
% Original IspTot intensity values versus time (before interpolating):
subplot(2,2,3);
plot(t_extendedTrack,I_extendedTrack,'gX-'); 
xlabel('t_{abs} (s)'); 
...
% SAVE current FIGURE as a .png (at screen size) and then close the figure window:
% Result figure files are saved in a new folder within the same directory where the input excel file was.
figName = strcat(new_folder_name,'_extendedTrack_',num2str(n_traj)); % name of figure file to save to. 
saveFigurePNG(new_folder_name,figName); % See saveFigurePNG.m.
...
% Parameters used in function findTrajects.m for sizes of regions used to
% calculate spot intensity:
processedTraj{n_trackInfo}.subarray_halfwidth = subarray_halfwidth; 
processedTraj{n_trackInfo}.inner_circle_radius = inner_circle_radius;
processedTraj{n_trackInfo}.gauss_mask_sigma = gauss_mask_sigma;
% Number of extra frames used to extend tracks and get background level:
processedTraj{n_trackInfo}.n_ExtraFrames = length(frameNum_Extra);
...
% -----------------
% Save extended tracks to another sheet in the excel file:
n_Extra = 11;
processedTraj{n_Extra} = results_extendedTrack;
...
dataForSheet11 = [fieldnames(processedTraj{n_Extra})'; num2cell(cell2mat(struct2cell(processedTraj{n_Extra})'))]; % vectors
...
xlswrite(output_filename,dataForSheet11,'Extended track data'); % write data to sheet 'Extended track data' in excel file.

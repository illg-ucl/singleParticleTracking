...
%For elliptical Gaussians, if arr is a square region around the brightest 
...
cols=find(mean(arr,1)>4*noiselevel/sqrt(size(z,2)));
rows=find(mean(arr,2)>4*noiselevel/sqrt(size(z,1)));
